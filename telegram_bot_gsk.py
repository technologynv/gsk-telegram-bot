...
GOSMS_URL = "http://127.0.0.1:8080/api/send"
...
def create_database():
    ...
    if cursor.fetchone()[0] == 0:
        test_data = [
            ...
            ("Сидоров", "Алексей", "Викторович", "ул. Гагарина 3, кв. 11", "+79028582408", 5000.00, 103),  # Телефон изменён!
        ...
        # Обновляем телефон для гаража 103 при каждом запуске (на случай, если база уже существует)
...
def send_sms(phone_number, message):
    ...
    try:
        ...
        with urllib.request.urlopen(req, json_data, timeout=15) as response:
            ...
            print(f"✅ Ответ от шлюза: {result}")
            if result.get('status') in ['success', 'ok', 'sent']:
                print(f"✅ СМС успешно отправлено на {phone_number}")
                return True
            else:
                print(f"❌ Ошибка от шлюза: {result}")
                return False
                
    except urllib.error.URLError as e:
        print(f"❌ Ошибка подключения к GoSMS: {e}")
        print("   Убедитесь, что приложение GoSMS запущено на телефоне")
        print("   Проверьте, что оно слушает порт 8080")
        return False
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == 'search':
        ...
        try:
            ...
            # Проверяем существование гаража
            owner = get_owner_by_garage(garage_num)
            if not owner:
                await update.message.reply_text(f"❌ Гараж №{garage_num} не найден")
                await start(update, context)
            ...
            await update.message.reply_text(
                ...
                f"(можно использовать шаблон, который я покажу ниже)",
                parse_mode='Markdown'
            )
        ...
        if owner:
            ...
            if success:
                ...
                await update.message.reply_text(
                    ...
                    f"• Приложение GoSMS не запущено\n"
                    f"• Нет баланса на сим-карте\n"
                    f"• Неправильный API ключ\n\n"
                    f"💡 Вы можете отправить сообщение вручную через приложение 'Сообщения'",
                    parse_mode='Markdown'
                )
        else:
...
if __name__ == "__main__":
    ...
    print(f"📞 Телефон гаража 103: +79028582408")
