    ...
    if action == 'search':
        ...
        try:
            ...
            if owner:
                ...
                await update.message.reply_text(f"Текущий долг для гаража №{garage_num}: {owner[5]:,.2f} руб.\nВведите новую сумму долга (отрицательное число - переплата):")
            else:
                await update.message.reply_text(f"❌ Гараж №{garage_num} не найден")
                context.user_data['action'] = None
        except ValueError:
        ...
        try:
            ...
            await update.message.reply_text(f"✅ Задолженность для гаража №{garage_num} обновлена!\nНовая сумма: {new_debt:,.2f} руб.")
            context.user_data['action'] = None
            await start(update, context)
        except ValueError:
